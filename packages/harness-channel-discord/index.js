/**
 * index.js — harness-channel-discord host 入口（薄壳，托管消息总线）
 *
 * Bot 网关。官方开放。需创建应用拿 token。
 * 消息总线（GatewayCore）与 logger 均从共享包 `@anarkhgatsby/deepseek-harness-core` 导入，
 * 本插件只做"平台门面"：注册 discord settings namespace、接适配器、注册 message 工具。
 */
...
import { DiscordAdapter } from "./lib/adapters/discord.mjs";
import { normalizeSettings } from "./lib/config.mjs";

export const name = "harness-channel-discord";
...
export const Config = z.object({
  ...
  statePath: z.string().default(join(homedir(), ".dsh", "discord-gateway-state.json")),
});

/** `discord` settings namespace schema。 */
...
const MANIFEST = {
  package: "harness-channel-discord",
  face: "host",
  schemas: [],
  invocations: [
    {
      id: "harness-channel-discord#discordGateway/getConfig",
      service: "discordGateway",
      namespace: "discordGateway",
      method: "getConfig",
      invocation: { kind: "direct" },
      parameters: [],
      result: { mode: "strict", typeSymbol: "harness-channel-discord#GatewayConfig", schema: getResultSchema },
    },
    {
      id: "harness-channel-discord#discordGateway/setConfig",
      service: "discordGateway",
      namespace: "discordGateway",
      method: "setConfig",
      invocation: { kind: "direct" },
      parameters: [
        {
          ...
          wire: "payload",
          source: "json",
          codec: { mode: "strict", typeSymbol: "harness-channel-discord#SetPayload", schema: setPayloadSchema },
        },
      ],
      result: { mode: "strict", typeSymbol: "harness-channel-discord#SetResult", schema: setResultSchema },
    },
  ],
...
export function apply(ctx, config) {
  const scope = ctx.settings.register("discord", GatewaySchema, {
    base: {
      routes: {},
      token: "",
  ...
  const log = createChannelLogger("discord", ctx.logger);

  const getConfig = () => normalizeSettings(scope.get());
  const adapter = new DiscordAdapter({ getConfig, log });
  ...
  ctx.effect(() => ctx.typert.register(MANIFEST), "harness-channel-discord: typert manifest");
  ...
  const messageTool = defineTool({
    name: "message",
    description: "通过 Discord 向频道或用户发送文本。用于主动通知/提醒用户。",
    parameters: {
      action: { type: "string", required: true, description: "操作类型，目前仅支持 send" },
      channel: { type: "string", required: true, description: "发送渠道：discord" },
  ...
  log.info("已注册全局 message 工具（Discord 发送）");
}
