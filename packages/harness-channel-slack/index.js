/**
 * index.js — harness-channel-slack host 入口（薄壳，托管消息总线）
 *
 ...
 * 本插件只做"平台门面"：注册 slack settings namespace、接适配器、注册 message 工具。
 */
...
import { SlackAdapter } from "./lib/adapters/slack.mjs";
import { normalizeSettings } from "./lib/config.mjs";

export const name = "harness-channel-slack";
...
export const Config = z.object({
  ...
  statePath: z.string().default(join(homedir(), ".dsh", "slack-gateway-state.json")),
});

/** `slack` settings namespace schema。 */
...
const MANIFEST = {
  package: "harness-channel-slack",
  face: "host",
  schemas: [],
  invocations: [
    {
      id: "harness-channel-slack#slackGateway/getConfig",
      service: "slackGateway",
      namespace: "slackGateway",
      method: "getConfig",
      invocation: { kind: "direct" },
      parameters: [],
      result: { mode: "strict", typeSymbol: "harness-channel-slack#GatewayConfig", schema: getResultSchema },
    },
    {
      id: "harness-channel-slack#slackGateway/setConfig",
      service: "slackGateway",
      namespace: "slackGateway",
      method: "setConfig",
      invocation: { kind: "direct" },
      parameters: [
        {
          ...
          wire: "payload",
          source: "json",
          codec: { mode: "strict", typeSymbol: "harness-channel-slack#SetPayload", schema: setPayloadSchema },
        },
      ],
      result: { mode: "strict", typeSymbol: "harness-channel-slack#SetResult", schema: setResultSchema },
    },
  ],
...
export function apply(ctx, config) {
  const scope = ctx.settings.register("slack", GatewaySchema, {
    base: {
      routes: {},
      botToken: "",
      appToken: "",
  ...
  const log = createChannelLogger("slack", ctx.logger);

  const getConfig = () => normalizeSettings(scope.get());
  const adapter = new SlackAdapter({ getConfig, log });
  ...
  ctx.effect(() => ctx.typert.register(MANIFEST), "harness-channel-slack: typert manifest");
  ...
  const messageTool = defineTool({
    name: "message",
    description: "通过 Slack 向会话发起人发送文本。用于主动通知/提醒用户。",
    parameters: {
      action: { type: "string", required: true, description: "操作类型，目前仅支持 send" },
      channel: { type: "string", required: true, description: "发送渠道：slack" },
  ...
  log.info("已注册全局 message 工具（Slack 发送）");
}
