/**
 * index.js — harness-channel-telegram host 入口（薄壳，托管消息总线）
 *
 * Bot API 长轮询。官方开放，最简单。需 BotFather 创建 bot 拿 token。
 * 消息总线（GatewayCore）与 logger 均从共享包 `@anarkhgatsby/deepseek-harness-core` 导入，
 * 本插件只做"平台门面"：注册 telegram settings namespace、接适配器、注册 message 工具。
 */
...
import { TelegramAdapter } from "./lib/adapters/telegram.mjs";
import { normalizeSettings } from "./lib/config.mjs";

export const name = "harness-channel-telegram";
...
export const Config = z.object({
  ...
  statePath: z.string().default(join(homedir(), ".dsh", "telegram-gateway-state.json")),
});

/** `telegram` settings namespace schema。 */
...
const MANIFEST = {
  package: "harness-channel-telegram",
  face: "host",
  schemas: [],
  invocations: [
    {
      id: "harness-channel-telegram#telegramGateway/getConfig",
      service: "telegramGateway",
      namespace: "telegramGateway",
      method: "getConfig",
      invocation: { kind: "direct" },
      parameters: [],
      result: { mode: "strict", typeSymbol: "harness-channel-telegram#GatewayConfig", schema: getResultSchema },
    },
    {
      id: "harness-channel-telegram#telegramGateway/setConfig",
      service: "telegramGateway",
      namespace: "telegramGateway",
      method: "setConfig",
      invocation: { kind: "direct" },
      parameters: [
        {
          ...
          wire: "payload",
          source: "json",
          codec: { mode: "strict", typeSymbol: "harness-channel-telegram#SetPayload", schema: setPayloadSchema },
        },
      ],
      result: { mode: "strict", typeSymbol: "harness-channel-telegram#SetResult", schema: setResultSchema },
    },
  ],
...
export function apply(ctx, config) {
  const scope = ctx.settings.register("telegram", GatewaySchema, {
    base: {
      routes: {},
      token: "",
  ...
  const log = createChannelLogger("telegram", ctx.logger);

  const getConfig = () => normalizeSettings(scope.get());
  const adapter = new TelegramAdapter({ getConfig, log });
  ...
  ctx.effect(() => ctx.typert.register(MANIFEST), "harness-channel-telegram: typert manifest");
  ...
  const messageTool = defineTool({
    name: "message",
    description: "通过 Telegram 向用户或群发送文本。用于主动通知/提醒用户。",
    parameters: {
      action: { type: "string", required: true, description: "操作类型，目前仅支持 send" },
      channel: { type: "string", required: true, description: "发送渠道：telegram" },
  ...
  log.info("已注册全局 message 工具（Telegram 发送）");
}
