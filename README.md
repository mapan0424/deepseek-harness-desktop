...
- 内置并验证最新社区插件矩阵：可视化配置 `0.1.7`、Core `0.1.4`、飞书 `0.1.2`、钉钉 `0.1.0`、企业微信 `0.1.4`、iMessage `0.1.5`、Insights `0.1.6` 和 Locale Pack `0.1.4`；
- 配置中心改为按运行时展示已安装渠道，钉钉使用亮色 / 深色官方图标资源；
- iMessage 回复在通过 Messages.app 发送前转换为易读纯文本，自动清理 Markdown 格式标记；
- Windows 安装包继续排除 iMessage，避免展示不可用入口；
- 所有内置插件已完成本机配置、收发消息和启动验证。
